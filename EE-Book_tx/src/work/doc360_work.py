...
from src.lib.parser.doc360_parser import Doc360ArticleParser, Doc360ColumnParser
import re
...
class Doc360Worker(object):
    ...
    def catch(account_id):
        ...
        for raw_front_page_index in range(0, max_page):
            ...
            urequest_url =  (request_url.format(raw_front_page_index,account_id,int(time.time()*1000)))
        ...
        while len(index_work_set) > 0 and re_catch_counter <= 20:
            ...
            for raw_front_page_index in index_work_set:
                ...
                list_p_list = soup.find_all('div', class_="list listwz1 font14")
        ...
        while len(index_work_set) > 0 and re_catch_counter <= 20:
            ...
            for article_url_index in index_work_set:
                ...
                article_info = Doc360ArticleParser(request_url_content).get_article_info()
