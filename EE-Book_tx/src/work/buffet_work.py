...
from src.lib.parser.buffet_parser import BuffettArticleParser, BuffettColumnParser
import re
...
class BuffettWorker(object):
    ...
    def catch(account_id):
        ...
        column_info = BuffettColumnParser(front_page_content).get_column_info()
        column_info[u'column_id'] = account_id
        column_info[u'title'] = "BerkshireHathaway"
        ...
        for raw_front_page_index in range(star_page, max_page):
            request_url = u'https://buffett.cnbc.com/{}-berkshire-hathaway-annual-meeting/'.format(account_id)
        ...
        while len(index_work_set) > 0 and re_catch_counter <= 20:
            ...
            for article_url_index in index_work_set:
                ...
                article_info = BuffettArticleParser(request_url_content).get_article_info()
