...
class XueQiuTodayArticleParser(ParserTools):
    def __init__(self, content):
        # self.dom = BeautifulSoup(content, 'html.parser')
    ...
    def get_article_info(self):
        ...
        try:
            ...
            if str(title_).startswith('$') or str(title_).strip() == '':
                return []


            data['title'] = title_.strip()
            u"""
            获得博文的内容
            :return:
            """
            ...
            data['author_avatar_url'] = ''
            data['author_gender'] = '0'
            # if int(article['fav_count']) > 5 or int(article['reply_count']) > 30 or int(article['retweet_count']) > 30:
            #     data['author_gender'] = '0'
            # else:
            #     return []
