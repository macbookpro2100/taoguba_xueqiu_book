...
class Path(object):
    base_path = unicode(os.path.abspath('.').decode(sys.getfilesystemencoding()))  # 初始地址,不含分隔符
    ...
    @staticmethod
    def get_pwd():
        path = unicode(os.path.abspath('.').decode(sys.getfilesystemencoding()))
        return path
    ...
    @staticmethod
    def init_base_path():
        Path.base_path = unicode(os.path.abspath('.').decode(sys.getfilesystemencoding()))
