...
from src.work.huxiu_work import HuXiuWorker
from src.work.weChat_work import WechatWorker
from src.work.xueqiu_work import XueQiuWorker
from src.work.huawei_work import HuaWeiWorker
from src.work.sinablog_work import SinaWorker
...
class Worker(object):
    ...
    def distribute(task):
        ...
        if task.get_task_type() == Type.author:
            ...
            HuaWeiWorker.catch(task.account_id)
        elif task.get_task_type() == Type.sina:
            SinaWorker.catch(task.account_id)
        elif task.get_task_type() == Type.wuxia:
            WuXiaWorker.catch(task.account_id)
        elif task.get_task_type() == Type.doc360:
